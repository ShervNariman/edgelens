...
function statusBadgeClass(status: IssueStatus | ChecklistStatus): string {
  switch (status) {
    case "done":
      return "border-emerald-600/25 bg-emerald-500/10 text-emerald-800";
    case "in_progress":
      return "border-sky-600/25 bg-sky-500/10 text-sky-800";
    case "in_review":
      return "border-violet-600/25 bg-violet-500/10 text-violet-800";
    case "partial":
      return "border-amber-600/25 bg-amber-500/10 text-amber-900";
    case "blocked":
...
function ChecklistIcon({ status }: { status: ChecklistStatus }) {
  ...
  if (status === "partial") {
    return <CircleDashed className="h-3.5 w-3.5 text-amber-700" />;
  }
  if (status === "blocked") {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto flex max-w-[1440px] flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        ...
        <p className="pb-4 font-mono text-[11px] text-muted-foreground">
          ...
          when sprint status changes. Manager playbook:{" "}
          <code className="text-foreground/80">docs/manager-loop.md</code>.
        </p>
      </main>
    </div>
  );
