  ...
  function submit(event: React.FormEvent) {
    ...
    if (!rationale.trim()) {
      ...
      feedback.push({
        ...
        detail: "Add a rationale before approving, blocking, or waiving.",
      });
      return;
    }
    setError(null);
    try {
      ...
      feedback.push({
        tone: action === "block" ? "warning" : "success",
        title: `${label} · audit event appended`,
        detail: "The decision is immutable in the audit trail.",
      });
      setRationale("");
    } catch {
      feedback.push({
        ...
        detail: "Try again. Nothing was written to the audit trail.",
      });
    }
  }
      ...
      <form onSubmit={submit} className="mt-4 space-y-4">
        <fieldset className="space-y-2" disabled={disabled}>
          ...
          {ACTIONS.map((item) => (
            <label
              key={item.id}
              className="flex cursor-pointer gap-3 rounded-lg border border-[var(--rr-line)] p-3 transition-colors has-[:checked]:border-[var(--rr-ink)] has-[:checked]:bg-black/[0.03]"
            >
              <input
                type="radio"
        ...
        <div className="space-y-2">
          ...
          <Textarea
            ...
            className="min-h-20 resize-y bg-[var(--rr-surface)]"
            disabled={disabled}
            required
          />
          {error ? (
