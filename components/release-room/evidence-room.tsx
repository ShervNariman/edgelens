...
function EvidenceRow({
  ...
  return (
    <li className="rounded-lg border border-[var(--rr-line)] bg-white/80 p-3 sm:p-4">
...
export function EvidenceRoom({ slug }: { slug: string }) {
  const {
    getRelease,
    refreshEvidence,
    captureEvidence,
  ...
  async function handleRefresh() {
    ...
    setRefreshing(false);
    setFlash("Evidence sources refreshed");
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link
          href="/"
          className="inline-flex items-center gap-1.5 text-sm text-[var(--rr-muted)] transition-colors hover:text-[var(--rr-ink)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--rr-focus)]"
        >
      ...
      >
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              ...
              {release.prUrl ? (
                <>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="underline decoration-[var(--rr-line)] underline-offset-2 hover:decoration-[var(--rr-ink)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--rr-focus)]"
                  >
                    PR #{release.prNumber}
                  </a>
                </>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {flash ? (
              <span className="text-xs font-medium text-[var(--rr-ready)]" role="status">
                {flash}
              </span>
            ) : null}
          </div>
        ...
        {release.riskClasses.length > 0 ? (
          <ul className="flex flex-wrap gap-2" aria-label="Risk classes">
            {release.riskClasses.map((cls) => (
              <li
                key={cls}
                className="rounded-md border border-[var(--rr-line)] bg-white/60 px-2 py-0.5 text-xs text-[var(--rr-ink)]"
              >
                {cls}
              </li>
            ))}
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
          ...
          >
            {grouped.map(({ group }) => (
              <a
                key={group}
                href={`#evidence-${group}`}
                className="shrink-0 rounded-md border border-[var(--rr-line)] bg-[var(--rr-surface)] px-3 py-1.5 text-xs font-medium text-[var(--rr-ink)] transition-colors hover:border-[var(--rr-ink)]/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--rr-focus)]"
              >
                {EVIDENCE_GROUP_LABELS[group]}
              </a>
            ))}
        ...
        <aside className="space-y-4 lg:sticky lg:top-6 lg:self-start">
          ...
          <section
            ...
            className="rounded-xl border border-[var(--rr-line)] bg-[var(--rr-surface)] p-4"
          >
            <h2
              id="activity-title"
